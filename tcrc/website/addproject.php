...
if($_SERVER['REQUEST_METHOD'] == "POST"){
  ...
  if(empty($office_err)
   ...
   && empty ($status_percentage_err))
   {
     ...
     $sql = "INSERT INTO project (office , research, projectTitle, projectNumber, projectDescription, DepartmentalCode, deptID, dateProposed, dateReceived, approved , signedRPA, WEPA, dateWithdrawn, dateCompleted , HostOrganizationName, hostOrganizationID, courseReq, notes, BFUser, BFActivity, facultySupervisorID, BFDateOfNote, dateProjectMatched, callNumber, staffID , staffCode , PClink , departmentCode , institutionID, status ,status_percentage) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";

     if($stmt = mysqli_prepare($link,$sql)){
       mysqli_stmt_bind_param($stmt,"sssississiiisssiisssssssisssiss",$office,$research,$projectTitle,$projectNumber,$projectDescription,$DepartmentalCode,$deptID,$dateProposed,$dateReceived,$approved,
       ...
       $dateProjectMatched,$callNumber,$staffID,$staffCode,$PClink,$departmentCode,$institutionID,$newStatus,$status_percentage);

       if(mysqli_stmt_execute($stmt)){
         header("location: projectprofile.php?id=".$stmt->insert_id);
       } else {
