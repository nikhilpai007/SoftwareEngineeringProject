...
if($_SERVER['REQUEST_METHOD'] == "POST"){
  ...
  && empty ($bio_err))
  {
    //TO DO
      ...
      $sql = 'INSERT INTO contact (title, firstName, lastName, workEmail, workPhone , workLocation , institutionID , contactType,TCRC,ULINKS,CLINKS,showFellow,fellowType) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)';

    if($stmt = mysqli_prepare($link,$sql)){

      mysqli_stmt_bind_param($stmt,"ssssssissssss", $title, $firstName, $lastName, $workEmail, $workPhone, $workLocation, $institutionID ,
      $contactType, $TCRC , $ULINKS, $CLINKS , $showFellow, $fellowType);


      if(mysqli_stmt_execute($stmt)){
          $contactID = $stmt ->insert_id;
        $sql = 'INSERT INTO contactInfo (contactID, personalEmail, address1,address2, city, province , pcode , country , phone,phoneAlt,bio) VALUES (?,?,?,?,?,?,?,?,?,?,?)';

        if($stmt = mysqli_prepare($link,$sql)){
          mysqli_stmt_bind_param($stmt,"issssssssss",$contactID, $personalEmail , $address1 , $address2 , $city , $province ,$pcode ,  $country, $phone , $phoneAlt,
          $bio);
