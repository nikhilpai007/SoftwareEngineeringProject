...
//If there are no errors with user input
  ...
  && empty ($cum_grade_err))
  {
    ...
    $sql = "UPDATE studentForm SET fname = ?, lname = ?, tNumber = ?, studNumber = ?, email = ?, college = ?, major = ?, address = ?, province = ?, city = ? , studentStatus = ?, notes = ?, mailConsentTCRC = ?, mailConsentULinks = ?, projectInterestID = ?, projectInterestID_2 = ?, projectInterestID_3 = ?,coursecode = ?,facultySupervisor = ?, CreditAchieved = ?, CumulativeAchieved = ?,deptPrereq = ?
    WHERE id = ?";
    //Prepare the query
    if($stmt = mysqli_prepare($link,$sql)){
      //BInd parameters

      mysqli_stmt_bind_param($stmt, "sssissssssssiiiiissiiii", $fname, $lname,$phone,$stud_num,$trent_email,$college,$major,$address,$province,$city,
      $stud_status,$source,$mail_TCRC,$mail_ULink, $proj_num1,$proj_num2,$proj_num3,$coursecode,$supervisor,
      $credits,$cum_grade,$dept_req,$requestedID);

      //Execute. If there are no problems, redirect
      if(mysqli_stmt_execute($stmt)){
          // Redirect to login page
          header("location: studentformentries.php");
...
if(isset($requestedID)){
  $query = "SELECT  * from studentForm where id = ?";
  if($stmt = mysqli_prepare($link,$query)){
    ...
    while($row = $result -> fetch_assoc()){
        ...
        $lname = $row['lname'];
        $phone = $row['tNumber'];
        $stud_num = $row['studNumber'];
        $trent_email = $row['email'];
        $college = $row['college'];
        $major = $row['major'];
        $address = $row['address'];
        $province = $row['province'];
        $city = $row['city'];
        $stud_status = $row['studentStatus'];
        $source = $row['notes'];
        $mail_TCRC = $row['mailConsentTCRC'];
        if($mail_TCRC = 0){
          $mail_TCRC = 'no';
        } else {
          $mail_TCRC = 'yes';
        }
        $mail_ULink = $row['mailConsentULinks'];
        if($mail_ULink = 0){
          $mail_ULink = 'no';
        } else {
          $mail_ULink = 'yes';
        }
        $proj_num1 = $row['projectInterestID'];
        $proj_num2 = $row['projectInterestID_2'];
        $proj_num3 = $row['projectInterestID_3'];
        $coursecode = $row['courseCode'];
        $supervisor = $row['facultySupervisor'];
        $credits = $row['CreditAchieved'];
        if($credits = 0){
          $credits = 'no';
        } else {
          $credits = 'yes';
        }
        $cum_grade = $row['CumulativeAchieved'];
        if($cum_grade = 0){
          $cum_grade = 'no';
        } else {
          $cum_grade = 'yes';
        }
        $dept_req = $row['deptPrereq'];
        if($dept_req = 0){
          $dept_req = 'no';
        } else {
          $dept_req = 'yes';
...
<body>
  <div class="limiter">
    <div class="container-login100" style="background-image: url('images/Login_background.jpg');">
      ...
      <form action="<?php echo $_SERVER["PHP_SELF"]."?id=".$requestedID; ?>" method="post">
        ...
        <div class="form-group <?php echo (!empty($proj_num1_err)) ? 'has-error' : ''; ?>">
          ...
          <select name = "proj_num1">
            ...
            <?php for($i = 0; $i < $counter; $i++){
              ...
              if($projectArray[$i]['2'] == $proj_num1)
                echo "<option selected value = '{$projectArray[$i]['2']}'>{$projectArray[$i]['2']} {$projectArray[$i]['1']}</option>";
              else
        ...
        <div class="form-group <?php echo (!empty($proj_num2_err)) ? 'has-error' : ''; ?>">
          ...
          <select name = "proj_num2">
            ...
            <?php for($i = 0; $i < $counter; $i++){
              //For all rows in the projects array, print out the value as an <option>
              if($projectArray[$i]['2'] == $proj_num2)
                echo "<option value = '{$projectArray[$i]['2']}' selected>{$projectArray[$i]['2']} {$projectArray[$i]['1']}</option>";
              else
                echo "<option value = '{$projectArray[$i]['2']}'>{$projectArray[$i]['2']} {$projectArray[$i]['1']}</option>";
            }?>
          </select>
        ...
        <div class="form-group <?php echo (!empty($dept_req_err)) ? 'has-error' : ''; ?>">
          ...
          <input type="radio" name="dept_req" value="yes" class="form-control" <?php if($dept_req == 'yes') echo 'checked = \'checked\''?>> Yes
